                            ...
                            <div class="panel-heading check">
                                <label class="w-100">
                                    ...
                                    <div class="check-content d-flex align-center">
                                        <span>{if $account->companyname}{$account->companyname} - {/if}{$account->fullName}</span>
                            ...
                            <div class="panel-collapse collapse {if $selectedAccountId == $account->id}show{/if}" data-input-collapse role="tabpanel">
                                <div class="panel-body">
                                    <address class="m-b-0x">
                                        ...
                                        {if $account->state}<span class="address-item">{$account->state}</span> <br />{/if}
                                        <span class="address-item">{$account->countryName}</span> <br />
                                        {if $account->phonenumber}<span class="address-item">{$account->phonenumber}</span><br/>{/if}
                                        {if $account->tax_id}<span class="address-item">{$taxLabel}: {$account->tax_id}</span>{/if}
                                    </address>
                                </div>
                            </div>
                        </div>
                    {/foreach}
                    <div class="panel panel-check {if !$selectedAccountId || !is_numeric($selectedAccountId)} checked{/if} {if $inExpressCheckout}disabled{/if} {if $RSThemes['pages'][$templatefile]['config']['hideCreateSubAccount'] == "1"}hidden{/if}" data-virtual-input>
                        <div class="panel-heading check">
                            <label>
