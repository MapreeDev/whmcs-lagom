...
{if $type == "switcher"}
    ...
    <label class="d-flex" data-inactive-services>
        <span data-inactive-services-text="hide" class="hidden">{$rslang->trans('hide_incactive_services.services.hide')|replace:"%s":$serviceCounter}</span>
        <span data-inactive-services-text="hidden">{$rslang->trans('hide_incactive_services.services.hidden')|replace:"%s":$serviceCounter}</span>
