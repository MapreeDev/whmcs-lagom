                                        ...
                                        <div class="cart-item-pricing dropdown" data-input-number-price>
                                            ...
                                            {foreach $allAvailableCycles[$num] as $cycle}
                                                    {if is_null($permittedBillingCycles) || in_array($cycle['cycle'], $permittedBillingCycles)}
                                                        <li>
                                                            {assign var="cycleQtyTemp" value=$cycle['price']|replace:$WHMCSCurrency.prefix:""|replace:$WHMCSCurrency.suffix:""}
                                                            {math equation="x*y" x=$cycleQtyTemp y=$product.qty assign="cycleQty" format="%.2f"}
                                                            
                                                            <a href="" onclick="selectProductPeriodInCart('{$num}', '{$cycle['cycle']}', '{$cycle['toFullString']|replace:$cycleQtyTemp:$cycleQty}'); return false;">
                                                                {$cycle['toFullString']|replace:$cycleQtyTemp:$cycleQty}
                                                            </a>
                                                        </li>
                                                    {/if}
                                                {/foreach}
