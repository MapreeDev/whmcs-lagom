...
</span>{extends file="adminarea/includes/layout.tpl"} 
        ...
        <div class="container">
            <div class="top">
                <div class="top__toolbar">
                    ...
                    {* <i class="btn__icon lm lm-arrow-thin-left"></i> *}
                    ...
                    <div class="top__title">
                        <h1 class="top__title-text">
                            <ul class="top__title-text breadcrumb breadcrumb--angle-separator ">
                                <li class="breadcrumb__item"><a class="breadcrumb__item-text" href="{$helper->url('Template@extensions',['templateName'=>$template->getMainName()])}">{$template->getName()}</a></li>
        ...
        <div class="t-c mob-t-c--full" data-table-container data-table-check-container>
            ...
            <div class="t-c__body t-c__body--boxed">
                <table class="t-c__table table mob-table--block" id="extensions-table" data-services-table data-search-input="#table-search" data-order='[3, "asc"]' data-clickable-rows="true" data-responsive="false">
                    ...
                    <tbody>
                        {foreach from=$extension->getStyleIntegrations() item=integration}
                            <tr>
                                <td class="cell-name">{$integration->getName()}</td>
                                <td class="cell-version">{$integration->getMinVersion()}</td>
                                <td class="cell-version">{$integration->getVersion()}</td>
                                ...
                                <td class="cell-actions">
                                    {if $integration->isActive()}
                                        <a href="{$helper->url('Template@extension',['templateName'=>$template->getMainName(),'extension'=>$extension->getLinkName(),'exaction'=>'deactivateIntegration','integration'=>$integration->getStyleName(),'integrationModule'=>$integration->getModule()])}" class="btn btn--sm btn--default btn--outline"><span class="btn__text">Dectivate</span></a>
                                    {else}
                                        <a href="{$helper->url('Template@extension',['templateName'=>$template->getMainName(),'extension'=>$extension->getLinkName(),'exaction'=>'activateIntegration','integration'=>$integration->getStyleName(),'integrationModule'=>$integration->getModule()])}" class="btn btn--sm btn--default btn--outline"><span class="btn__text">Activate</span></a>
                                    {/if}
                                </td>
                            </tr>
                        {/foreach}
